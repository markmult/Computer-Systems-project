...
void mpu9250_get_data(I2C_Handle *i2c, float *ax, float *ay, float *az, float *gx, float *gy, float *gz) {
	...
	// 2. STORE THE VALUES INTO THE ARRAY data[7]
	    data[0] = rawData[0] << 8  |  (rawData[1] & 0xFF);
	    data[1] = rawData[2] << 8  |  (rawData[3] & 0xFF);
	    data[2] = rawData[4] << 8  |  (rawData[5] & 0xFF);
	    //data[3] = rawData[6] << 8  |  (rawData[7] & 0xFF);
	    data[4] = rawData[8] << 8  |  (rawData[9] & 0xFF);
	    data[5] = rawData[10] << 8  |  (rawData[11] & 0xFF);
	    data[6] = rawData[12] << 8  |  (rawData[13] & 0xFF);
